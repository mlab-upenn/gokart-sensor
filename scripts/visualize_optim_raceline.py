...
def read_wp(config_folder, location, WP_FILE_NAME='wp.csv'):
...
def show_optim_raceline(config_folder, LOCATION):
    # wp_x, wp_y = read_wp(config_folder, LOCATION, WP_FILE_NAME='wp.csv')
    raceline_path = os.path.join(config_folder, LOCATION, 'optim_traj.csv')
    raceline = np.loadtxt(raceline_path, delimiter=';', skiprows=3)
    raceline_x = raceline[:, 1]
    raceline_y = raceline[:, 2]
    visualize_curvature_for_wp(raceline_x, raceline_y)



if __name__ == "__main__":
    # visualize_curvature_for_wp(config_folder, LOCATION)
    show_optim_raceline(config_folder, LOCATION)

...
path = "/home/felix/gokart-sensor/catkin_ws_evaluation/src/evaluation/src/support/2023-02-01-17-00-08"
...
def callback(marker_array):
    ...
    if len(marker_array.markers) > 0:
        if marker_array.markers[0].header.stamp.secs >= float(timestamp_sec):
            if marker_array.markers[0].header.stamp.nsecs >= float(timestamp_nsec) + 0: # 275000000 OpenPCDet late fusion     150000000 Darknet

                if i < len(filelist) - 1:
                    print("Ros timestamp: " + str(marker_array.markers[0].header.stamp.secs) + "." + str(marker_array.markers[0].header.stamp.nsecs))
                    print("File timestamp: " + str(timestamp_sec) + "." + str(timestamp_nsec))
                    ...
                    print("Annotation: ", annotation_array)
                    ...
                    for j in range(annotation_array.shape[0]):
                        ...
                        marker_array_pub = MarkerArray()
                        marker = Marker()
                        marker.header.frame_id = "os_sensor"
                        marker.id = 0
                        marker.ns = "detections"
                        marker.action = Marker.DELETEALL
                        marker_array_pub.markers.append(marker)
                        pub_marker_array.publish(marker_array_pub)
                        ...
                        for k in range(marker_array.shape[0]):
                            ...
                            marker.pose.position.x = marker_array[k][0]
                            marker.pose.position.y = marker_array[k][1]
                    ...
                    print("ROS execution time: " + str(time.time() - start_time) + " seconds")
    ...
    if i == len(filelist) - 1 and end_eval_seen == False:
        ...
        if abs_num_false_negatives > 0:
            print("Absolute Accuracy for False Negatives: " + str(abs_accuracy_FN / abs_num_false_negatives))
...
def evaluation():
    ...
    rospy.init_node('evaluation_5', anonymous=True)

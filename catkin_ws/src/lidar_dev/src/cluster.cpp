...
int num_expected_points(const pcl::PointXYZ &centre) {
    ...
    static double rv = 21.5 * 2 * M_PI / (360 * 32);  // angular resolution vertical
...
void cloud_cluster_cb(const sensor_msgs::PointCloud2ConstPtr &obstacles_msg, const sensor_msgs::PointCloud2ConstPtr &ground_msg)
{
    ...
    for (auto it = cluster_indices.begin(); it != cluster_indices.end(); ++it)
    {
        ...
        std::cout << zvals.front() << endl;

        double avg_z = (zvals.front() - zvals.back()); 

        for (auto i: testcloud2.points)
        {
            // std::cout << i << ' ' << std::endl;
            double c_z = i.z;
            if(c_z > zvals.front() - 0.25 * avg_z)
            {
                perc_75_100 += 1;
            }
            else if (c_z <= zvals.front() - 0.25 * avg_z && c_z > zvals.front() - 0.5 * avg_z)
            {
                perc_50_75 += 1;
            }
            else if (c_z <= zvals.front() - 0.5 * avg_z && c_z > zvals.front() - 0.75 * avg_z)
            {
                perc_25_50 += 1;
            }
            else
        ...
        cout << "Front" << zvals.front() << endl;
        cout << "Back" << zvals.back() << endl;
        cout << avg_z << endl;


        if(perc_75_100 > perc_25_50 || perc_75_100 > perc_0_25 || perc_50_75 > perc_0_25)
        {

...
def rslidar_callback(msg):
    ...
    scores, dt_box_lidar, types, pred_dict, inference_time = proc_1.run(np_p, frame)
    for i, score in enumerate(scores):
        if score>threshold:
            select_boxs.append(dt_box_lidar[i])
            select_types.append(pred_dict['name'][i])
    if(len(select_boxs)>0):
        proc_1.pub_rviz.publish_3dbox(np.array(select_boxs), -1, inference_time, pred_dict['name'])
...
class Processor_ROS:
    ...
    def run(self, points, frame):
        ...
        print("Mean inferen time: ", mean_inference_time)
        ...
        return scores, boxes_lidar, types, pred_dict, inference_time
